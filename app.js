...
{e:‘🍜’,n:‘餐飲’,subs:[{e:‘🌅’,n:‘早餐’},{e:‘☀️’,n:‘午餐’},{e:‘🌙’,n:‘晚餐’},{e:‘🧋’,n:‘飲料’},{e:‘🍱’,n:‘外帶’},{e:‘🥂’,n:‘聚餐’},{e:‘🍦’,n:‘甜點’}]},
...
const PAYER_AVATARS = [‘🙋’,‘💑’];
const CHART_COLORS = [’#FF6B6B’,’#FFD93D’,’#6BCB77’,’#4D96FF’,’#C77DFF’,’#FF9A3C’,’#00C9A7’,’#F72585’,’#4CC9F0’,’#FF6BB5’];
...
// ── Config ────────────────────────────────────────────────
const Config = {
get scriptUrl() { return localStorage.getItem(‘su’) || ‘https://script.google.com/macros/s/AKfycbyoXJdQZ2M-QauvyyQvoahvTyDLfaX-7P9Z5eIt9X57AmDito1FCyfTr1ft4L8XCtvC/exec’; },
...
return `<div class="cur-item${isSel?' sel':''}" onclick="selectCur('${c.code}')"> <span class="cur-flag">${c.flag}</span> <div class="cur-info"><div class="cur-code">${c.code}</div><div class="cur-name">${c.name}</div></div> ${r ? `<span class="cur-rate">${r}</span>` : ''}<span class="cur-ck2">✓</span> </div>`;
}).join(’’);
}
...
document.getElementById(‘payerGrid’).innerHTML = names.map((n, i) => ` <div class="payer-item${State.selPayer === i ? ' sel' : ''}" onclick="pickPayer(${i})"> <span class="payer-avatar">${PAYER_AVATARS[i]}</span> <span class="payer-name">${n}</span> </div>`).join(’’);
}
...
document.getElementById(‘bigGrid’).innerHTML = cats.map((c, i) => ` <div class="bc${State.selBig === i ? ' ' + sc : ''}" onclick="pickBig(${i})"> <div class="bc-e">${c.e}</div><div class="bc-n">${c.n}</div> </div>`).join(’’);
}
function pickBig(i) {
...
document.getElementById(‘subGrid’).innerHTML = cat.subs.map((s, i) => ` <div class="si${State.selSub === i ? ' ' + sc : ''}" onclick="pickSub(${i})"> <span class="si-e">${s.e}</span><span class="si-n">${s.n}</span> </div>`).join(’’);
}
...
return `<div class="cr${isSel?' sel':''}" onclick="pickCard(${i})"> <span class="cr-ic">${c.emoji||'💳'}</span> <div class="cr-info"> <div class="cr-nm">${c.name}</div> <div class="cr-num">•••${last5}</div> </div> <span class="cr-ck">✓</span> </div>`;
}).join(’’);
}
...
document.getElementById(id).innerHTML = ` <div class="ip-amt ${typeClass()}">${amtLabel()}</div> <span class="ip-sep">·</span> <div class="ip-meta">${parts.join(' · ')}</div>`;
}
...
document.getElementById(‘cfCard’).innerHTML = `<div class="cf-row"><span class="cf-lbl">類型</span><span class="cf-val ${typeClass()}">${State.type==='exp'?'💸 支出':'💚 收入'}</span></div> <div class="cf-row"><span class="cf-lbl">金額</span><span class="cf-val ${typeClass()}">${amtLabel()}</span></div> ${isForeign ?`<div class="cf-row"><span class="cf-lbl">台幣</span><span class="cf-val">NT$${twd.toLocaleString(‘zh-TW’)}</span></div>` : ''} <div class="cf-row"><span class="cf-lbl">日期</span><span class="cf-val">${getDateStr()}</span></div> <div class="cf-row"><span class="cf-lbl">誰</span><span class="cf-val">${payerLabel()}</span></div> <div class="cf-row"><span class="cf-lbl">類別</span><span class="cf-val">${catLabel()}</span></div> <div class="cf-row"><span class="cf-lbl">${State.type==='exp'?'卡別':'帳戶'}</span><span class="cf-val">${cardLabel()}${last5?' <span style="color:var(--ink2);font-size:12px">'+last5+'</span>':''}</span></div>`;
}
...
html += `<div class="${cls}" onclick="pickRecDay('${dateKey}')"> <div class="cv-d-num">${day}</div> ${hasExp ? '<div class="cv-dot"></div>' : ''} ${hasInc ? '<div class="cv-dot inc"></div>' : ''} ${total > 0 ? `<div class="cv-day-amt">${total >= 10000 ? (total/1000).toFixed(0)+‘k’ : total.toLocaleString(‘zh-TW’)}</div>` : ''} </div>`;
}
const rem = 42 - (startDow + last.getDate());
...
const total = recs.reduce((a,r)=>(r.type===‘exp’||r.type===‘支出’)?a+(parseFloat(r.twdAmount)||0):a, 0);
return `<div class="rec-date-group"> <div class="rec-date-lbl">${lbl}<span>${total>0?'$'+Math.round(total).toLocaleString('zh-TW'):''}</span></div> ${recs.map(r=>recItemHTML(r)).join('')} </div>`;
}).join(’’);
}
...
document.getElementById(‘pieChartLegend’).innerHTML = sorted.map(([lbl,amt],i)=>` <div class="legend-row"> <span class="legend-dot" style="background:${CHART_COLORS[i]}"></span> <span class="legend-lbl">${lbl}</span> <span class="legend-pct">${(amt/total*100).toFixed(1)}%</span> <span class="legend-amt">$${Math.round(amt).toLocaleString('zh-TW')}</span> </div>`).join(’’);
const days = getPeriodDays();
...
const total = sorted.reduce((a,[,v])=>a+v,0)||1;
document.getElementById(‘payerLegend’).innerHTML = sorted.map(([lbl,amt],i)=>` <div class="legend-row"> <span class="legend-dot" style="background:${payerColors[i]}"></span> <span class="legend-lbl">${lbl}</span> <span class="legend-pct">${(amt/total*100).toFixed(1)}%</span> <span class="legend-amt">$${Math.round(amt).toLocaleString('zh-TW')}</span> </div>`).join(’’);
}
...
el.innerHTML = sorted.map(([lbl,amt],i)=>` <div class="bar-row"> <div class="bar-lbl">${lbl}</div> <div class="bar-track"><div class="bar-fill" style="width:${(amt/max*100).toFixed(0)}%;background:${colors[i%colors.length]}"></div></div> <div class="bar-val">$${Math.round(amt).toLocaleString('zh-TW')}</div> </div>`).join(’’);
}
...
el.innerHTML = State.fixedExpenses.map(f => ` <div class="fixed-item"> <div class="fixed-info"><div class="fixed-name">${f.name}</div><div class="fixed-meta">每月 ${f.day} 日</div></div> <div class="fixed-amt">NT$${f.amt.toLocaleString('zh-TW')}</div> <div class="fixed-del" onclick="deleteFixed(${f.id})">✕</div> </div>`).join(’’);
}
...
if (!card) return;
// Only prompt if lock is configured
if (Config.lockType !== ‘none’ && Config.lockSecret) {
const pwd = prompt(‘請輸入密碼以查看卡片資訊：’);
if (pwd !== Config.lockSecret) return toast(‘密碼錯誤’, ‘err’);
}
showCardView(card);
}
...
document.getElementById(‘cardViewDisplay’).innerHTML = ` <div class="credit-card-visual"> <div> <div class="cc-chip">▪▪</div> <div class="cc-number">${card.number || '•••• •••• •••• ••••'}</div> </div> <div class="cc-bottom"> <div><div class="cc-label">持卡人</div><div class="cc-value">${card.name}</div></div> <div><div class="cc-label">有效期限</div><div class="cc-value">${card.expiry || '••/••'}</div></div> <div><div class="cc-label">CVV</div><div class="cc-value">${cvv || '•••'}</div></div> </div> <div class="cc-emoji">${card.emoji || '💳'}</div> </div>`;
}
...
return `<div class="card-cfg-item"> <span class="card-cfg-emoji">${c.emoji||'💳'}</span> <div class="card-cfg-info"> <div class="card-cfg-name">${c.name}</div> <div class="card-cfg-num">•••• ${last4}</div> </div> <div class="card-cfg-actions"> <span onclick="viewCard(${c.id})" class="card-cfg-btn">👁</span> <span onclick="deleteCard(${c.id})" class="card-cfg-btn del">✕</span> </div> </div>`;
}).join(’’);
}
...
document.getElementById(‘catEditorList’).innerHTML = cats.map((c, i) => ` <div class="cat-editor-item"> <span class="ce-emoji">${c.e}</span> <span class="ce-name">${c.n}</span> <span class="ce-subs">${c.subs.length} 項細項</span> <span class="ce-edit" onclick="openSubEditor(${i})">✏️</span> <span class="ce-del" onclick="deleteCat(${i})">✕</span> </div>`).join(’’);
}
...
document.getElementById(‘subEditorList’).innerHTML = subs.map((s, i) => ` <div class="cat-editor-item"> <span class="ce-emoji">${s.e}</span> <span class="ce-name">${s.n}</span> <span class="ce-del" onclick="deleteSub(${catIdx},${i})">✕</span> </div>`).join(’’);
}
